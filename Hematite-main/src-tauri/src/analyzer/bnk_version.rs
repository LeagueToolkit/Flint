        ...
        if section_id_str == "BKHD" {
            ...
            if offset + 12 <= data.len() {
                ...
                let reason = if should_remove {
                    format!("Version {} not in allowed list {:?}", version, ALLOWED_BNK_VERSIONS)
                } else {
...
/// Analyze multiple BNK chunks and return those that should be removed
pub fn filter_removable_bnks(bnk_data: &[(String, Vec<u8>)]) -> Vec<(String, BnkInfo)> {
    bnk_data
        .iter()
        .map(|(path, data)| {
        ...
        .filter(|(_, info)| info.should_remove)
        .collect()
}
